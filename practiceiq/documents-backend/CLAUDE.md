# PracticeIQ Documents — Telegram bot service

## Purpose
This directory contains **only the Python FastAPI service** that runs the Telegram bot ("Shalini") and the daily follow-up scheduler. It is one half of the PracticeIQ Documents feature; the user-facing UI lives at `practiceiq/app/(app)/documents/`.
...
| Concern | Lives in |
|---|---|
| Telegram polling/webhook, document classification, OCR-light text extraction, Hinglish replies, daily follow-up cron | `practiceiq/documents-backend/` (this dir) |
...
## Directory
```
practiceiq/documents-backend/
├── CLAUDE.md                       ← this file
├── main.py                         FastAPI + bot startup + scheduler boot
├── requirements.txt
├── .env.example
├── db/
│   └── schema.sql                  Reference pointer — canonical schema lives in Supabase migrations
├── agents/
...
The service loads the **repo-root** `.env` (three levels up from `main.py`).
...
# Cross-service
DOCUMENTS_BACKEND_URL=http://localhost:8000   # Used by Next.js to proxy "Ask Shalini" + "Send reminder" to this service
```
...
The canonical schema lives in Supabase migration `workflowiq_schema_v1_drop_and_rebuild`. Tables consumed by this service:

| Table | Notes |
|---|---|
| `practiceiq_clients` | telegram_chat_id (UNIQUE), telegram_username, telegram_first_name, consent_given, consent_at, filing_period, entity_type |
| `practiceiq_settings` | ca_telegram_chat_id |
| `practiceiq_documents` | source, source_ref, uploaded_by, mime_type, doc_type, filing_period, ocr_text, embedding vector(1536), status, verified_by, verified_at, rejection_reason, deleted_at, retention_until, is_sensitive. HNSW + FTS indexes. |
...
### Storage
Bucket `practiceiq-docs`. Path: `{owner_user_id}/{client_id}/{filing_period}/{doc_type}/{ts}_{filename}`. Signed URLs (5-min) generated by Next.js on demand.

### RLS posture (V1)
RLS enabled with per-`owner_user_id` policies on all PracticeIQ tables. Backend uses **service role key** which bypasses RLS.

---

## Build status
Schema, RLS, and indexes complete. Python service implementation is partially stubbed (`tools/db_tools.py` returns `NotImplementedError` for most functions); finish wiring it before standing up the bot.
