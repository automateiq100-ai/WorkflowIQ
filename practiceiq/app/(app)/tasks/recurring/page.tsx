...
export default function RecurringPage() {
  ...
  async function load() {
    ...
    const [t, c] = await Promise.all([
      fetch(api('/api/practiceiq/recurring')).then(r => r.json()),
  ...
  async function spawn() {
    setSpawning(true);
    setSpawnMsg(null);
    const res = await fetch(api('/api/practiceiq/recurring/spawn'), { method: 'POST' }).then(r => r.json());
    setSpawning(false);
  ...
  async function toggleActive(t: RecurringTemplate) {
    await fetch(api(`/api/practiceiq/recurring/${t.id}`), {
      method: 'PATCH',
  ...
  async function del(t: RecurringTemplate) {
    ...
    await fetch(api(`/api/practiceiq/recurring/${t.id}`), { method: 'DELETE' });
    load();
  }

  async function applyPreset(preset: typeof RECURRING_PRESETS[number]) {
    if (clients.length === 0) { alert('Add a client first before applying a preset.'); return; }
    const clientId = prompt(`Apply "${preset.title}" to which client?\n\n${clients.map((c, i) => `${i + 1}. ${c.name}`).join('\n')}\n\nEnter number:`);
    if (!clientId) return;
    const idx = parseInt(clientId) - 1;
    if (idx < 0 || idx >= clients.length) return;
    await fetch(api('/api/practiceiq/recurring'), {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ ...preset, client_id: clients[idx].id, active: true }),
    });
    load();
  }
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl mb-1" style={{ fontFamily: 'var(--font-dm-serif)', color: 'var(--text1)' }}>Recurring Templates</h1>
          <p className="text-sm" style={{ color: 'var(--text2)' }}>Auto-generate compliance tasks on a cadence (GST, TDS, ROC, ITR)</p>
        </div>
        <div className="flex gap-2">
          <button onClick={spawn} disabled={spawning || templates.filter(t => t.active).length === 0} className="px-4 py-2 rounded-lg text-sm font-medium" style={{ background: 'var(--teal)', color: '#000', opacity: spawning ? 0.5 : 1 }}>
      ...
      {spawnMsg && <div className="mb-4 px-4 py-2 rounded-lg text-sm" style={{ background: 'var(--bg3)', color: 'var(--teal)' }}>{spawnMsg}</div>}

      <div className="mb-8">
        <div className="text-xs uppercase mb-3" style={{ color: 'var(--text3)' }}>Quick presets — click to apply</div>
        <div className="flex flex-wrap gap-2">
          {RECURRING_PRESETS.map(p => (
            <button key={p.title} onClick={() => applyPreset(p)} className="px-3 py-1.5 rounded-lg text-xs" style={{ background: 'var(--bg2)', border: '1px solid var(--border)', color: 'var(--text2)' }}>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border p-8 text-center" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
          <div className="text-sm" style={{ color: 'var(--text3)' }}>No recurring templates yet. Click a preset above to add one.</div>
        </div>
      ) : (
        <div className="rounded-xl border overflow-hidden" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
          <table className="w-full text-sm">
            <thead><tr style={{ background: 'var(--bg3)' }}><Th>Title</Th><Th>Client</Th><Th>Cadence</Th><Th>When</Th><Th>Last spawned</Th><Th>Active</Th><Th></Th></tr></thead>
            <tbody>
              {templates.map(t => {
                ...
                return (
                  <tr key={t.id} style={{ borderTop: '1px solid var(--border)' }}>
                    ...
                    <td className="px-4 py-3" style={{ color: 'var(--text2)' }}>Day {t.day_of_month ?? '?'}{t.month_of_year ? `, month ${t.month_of_year}` : ''}</td>
                    <td className="px-4 py-3" style={{ color: 'var(--text3)' }}>{t.last_spawned_for ?? '—'}</td>
                    <td className="px-4 py-3">
                      <button onClick={() => toggleActive(t)} className="text-xs px-2 py-0.5 rounded-full" style={{ background: t.active ? 'rgba(15,212,160,0.15)' : 'var(--bg3)', color: t.active ? 'var(--teal)' : 'var(--text3)' }}>
...
function TemplateForm({ initial, clients, onClose, onSaved }: { initial?: RecurringTemplate; clients: Client[]; onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState<Partial<RecurringTemplate>>(initial ?? { cadence: 'monthly', active: true, day_of_month: 11 });
  const [saving, setSaving] = useState(false);

  async function save() {
    setSaving(true);
    const url = initial?.id ? api(`/api/practiceiq/recurring/${initial.id}`) : api('/api/practiceiq/recurring');
    const method = initial?.id ? 'PATCH' : 'POST';
    await fetch(url, {
      method,
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ ...form, client_id: form.client_id || null, fee_amount: form.fee_amount === undefined || (form.fee_amount as unknown) === '' ? null : Number(form.fee_amount), day_of_month: form.day_of_month != null ? Number(form.day_of_month) : null, month_of_year: form.month_of_year != null ? Number(form.month_of_year) : null }),
    });
    setSaving(false);
    onSaved();
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.6)' }} onClick={onClose}>
      <div className="rounded-xl border p-6 w-full max-w-xl" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }} onClick={e => e.stopPropagation()}>
        <h2 className="text-lg mb-4" style={{ fontFamily: 'var(--font-dm-serif)', color: 'var(--text1)' }}>{initial?.id ? 'Edit Template' : 'New Recurring Template'}</h2>
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Day of month (1–31)"><input type="number" min={1} max={31} value={form.day_of_month ?? ''} onChange={e => setForm({ ...form, day_of_month: Number(e.target.value) })} style={inputStyle} /></Field>
            <Field label="Month of year (1–12)"><input type="number" min={1} max={12} value={form.month_of_year ?? ''} onChange={e => setForm({ ...form, month_of_year: Number(e.target.value) })} style={inputStyle} /></Field>
            <Field label="Fee (₹)"><input type="number" value={form.fee_amount ?? ''} onChange={e => setForm({ ...form, fee_amount: Number(e.target.value) })} style={inputStyle} /></Field>
