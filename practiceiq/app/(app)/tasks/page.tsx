      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          <div className="flex rounded-lg overflow-hidden" style={{ border: '1px solid var(--border)' }}>
            <button onClick={() => setView('kanban')} className="px-3 py-2 text-xs" style={{ background: view === 'kanban' ? 'var(--bg3)' : 'var(--bg2)', color: 'var(--text1)' }}>Kanban</button>
            <button onClick={() => setView('list')} className="px-3 py-2 text-xs" style={{ background: view === 'list' ? 'var(--bg3)' : 'var(--bg2)', color: 'var(--text1)' }}>List</button>
          </div>
...
function KanbanBoard({ tasks, clients, onMove, onClick }: { tasks: Task[]; clients: Client[]; onMove: (t: Task, s: TaskStatus) => void; onClick: (t: Task) => void }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
      {COLUMNS.map(col => {
        ...
        return (
          <div key={col.key} className="rounded-lg p-3 min-h-[400px]" style={{ background: 'var(--bg2)', border: '1px solid var(--border)' }}
            onDragOver={e => e.preventDefault()}
            onDrop={() => { if (!dragId) return; const t = tasks.find(x => x.id === dragId); if (t && t.status !== col.key) onMove(t, col.key); setDragId(null); }}>
            ...
            {items.map(t => {
              ...
              return (
                <div key={t.id} draggable onDragStart={() => setDragId(t.id)} onClick={() => onClick(t)}
                  className="rounded-lg p-3 mb-2 cursor-pointer" style={{ background: 'var(--bg3)', border: '1px solid var(--border)' }}>
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <div className="text-sm" style={{ color: 'var(--text1)' }}>{t.title}</div>
                    <span className="text-[10px] uppercase shrink-0 px-1.5 rounded" style={{ background: 'var(--bg4)', color: PRIORITY_COLOR[t.priority] }}>{t.priority}</span>
                  </div>
  ...
  return (
    <div className="rounded-xl border overflow-hidden" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
      <table className="w-full text-sm">
        <thead><tr style={{ background: 'var(--bg3)' }}><Th>Title</Th><Th>Client</Th><Th>Status</Th><Th>Priority</Th><Th>Due</Th><Th>Fee</Th></tr></thead>
        <tbody>
          {tasks.map(t => {
            const client = clients.find(c => c.id === t.client_id);
            const overdue = t.due_date && new Date(t.due_date) < new Date() && t.status !== 'done';
...
function TaskForm({ initial, clients, onClose, onSaved }: { initial?: Task; clients: Client[]; onClose: () => void; onSaved: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.6)' }} onClick={onClose}>
      <div className="rounded-xl border p-6 w-full max-w-xl" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }} onClick={e => e.stopPropagation()}>
        <h2 className="text-lg mb-4" style={{ fontFamily: 'var(--font-dm-serif)', color: 'var(--text1)' }}>{initial?.id ? 'Edit Task' : 'New Task'}</h2>
