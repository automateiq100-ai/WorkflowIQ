...
const STATUS_COLOR: Record<InvoiceStatus, string> = {
  draft: 'var(--text3)', sent: 'var(--blue)', paid: 'var(--green)', overdue: 'var(--red)',
};
...
export default function InvoicesPage() {
  ...
  async function load() {
    ...
    const [i, c] = await Promise.all([
      fetch(api('/api/practiceiq/invoices')).then(r => r.json()),
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl mb-1" style={{ fontFamily: 'var(--font-dm-serif)', color: 'var(--text1)' }}>Invoices</h1>
          <p className="text-sm" style={{ color: 'var(--text2)' }}>{invoices.length} invoice{invoices.length === 1 ? '' : 's'}</p>
        </div>
        <Link href="/invoices/new" className="px-4 py-2 rounded-lg text-sm font-medium" style={{ background: 'var(--purple)', color: '#fff' }}>
          + Create Invoice
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border overflow-hidden" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
          <table className="w-full text-sm">
            <thead><tr style={{ background: 'var(--bg3)' }}><Th>Number</Th><Th>Client</Th><Th>Issue Date</Th><Th>Due</Th><Th>Total</Th><Th>Status</Th><Th></Th></tr></thead>
            <tbody>
              {invoices.map(i => {
                ...
                return (
                  <tr key={i.id} style={{ borderTop: '1px solid var(--border)' }}>
                    ...
                    <td className="px-4 py-3">
                      <span className="text-xs px-2 py-0.5 rounded-full uppercase" style={{ background: 'var(--bg3)', color: STATUS_COLOR[i.status] }}>{i.status}</span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <Link href={`/invoices/${i.id}`} className="text-xs" style={{ color: 'var(--purple)' }}>View →</Link>
                    </td>
                  </tr>
                );
              })}
