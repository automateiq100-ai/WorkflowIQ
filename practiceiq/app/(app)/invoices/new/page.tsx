...
export default function NewInvoicePage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(api('/api/practiceiq/tasks?status=done')).then(r => r.json()),
      fetch(api('/api/practiceiq/settings')).then(r => r.json()),
    ]).then(([c, t, s]) => {
      setClients(c.data ?? []);
      setTasks(t.data ?? []);
  ...
  function updateItem(i: number, patch: Partial<InvoiceLineItem>) {
    setItems(prev => { const next = [...prev]; next[i] = { ...next[i], ...patch }; next[i].amount = Number(next[i].qty) * Number(next[i].rate); return next; });
  }

  function importFromTask(taskId: string) {
    const t = tasks.find(x => x.id === taskId);
    if (!t) return;
    setItems(prev => [...prev.filter(i => i.description || i.amount), { description: t.title, qty: 1, rate: Number(t.fee_amount ?? 0), amount: Number(t.fee_amount ?? 0), task_id: t.id }]);
  ...
  async function save() {
    setSaving(true);
    const res = await fetch(api('/api/practiceiq/invoices'), {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ client_id: clientId || null, issue_date: issueDate, due_date: dueDate || null, line_items: items.filter(i => i.description), subtotal, tax_amount: taxAmount, total, status: 'draft', notes: notes || null }),
    }).then(r => r.json());
    setSaving(false);
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <Link href="/invoices" className="text-xs mb-4 inline-block" style={{ color: 'var(--text3)' }}>← All invoices</Link>
      <h1 className="text-2xl mb-6" style={{ fontFamily: 'var(--font-dm-serif)', color: 'var(--text1)' }}>New Invoice</h1>
      ...
      <div className="rounded-xl border p-6 mb-4" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
          ...
          <div className="mb-4 p-3 rounded-lg" style={{ background: 'var(--bg3)' }}>
            ...
            <div className="flex flex-wrap gap-1.5">
              {completedTasksForClient.map(t => (
                <button key={t.id} onClick={() => importFromTask(t.id)} className="px-2 py-1 rounded text-xs" style={{ background: 'var(--bg4)', color: 'var(--text2)' }}>
        ...
        <table className="w-full text-sm mb-3">
          ...
          <tbody>
            {items.map((it, i) => (
              <tr key={i}>
                <td className="pr-2 py-1"><input value={it.description} onChange={e => updateItem(i, { description: e.target.value })} style={input} /></td>
                <td className="pr-2 py-1"><input type="number" value={it.qty} onChange={e => updateItem(i, { qty: Number(e.target.value) })} style={{ ...input, textAlign: 'right' }} /></td>
                <td className="pr-2 py-1"><input type="number" value={it.rate} onChange={e => updateItem(i, { rate: Number(e.target.value) })} style={{ ...input, textAlign: 'right' }} /></td>
                <td className="text-right py-1" style={{ color: 'var(--text1)' }}>₹{it.amount.toLocaleString('en-IN')}</td>
                <td className="text-center"><button onClick={() => setItems(prev => prev.filter((_, idx) => idx !== i))} style={{ color: 'var(--red)' }}>×</button></td>
              </tr>
            ))}
          </tbody>
        </table>
        <button onClick={() => setItems(prev => [...prev, { description: '', qty: 1, rate: 0, amount: 0 }])} className="text-xs" style={{ color: 'var(--purple)' }}>+ Add line item</button>
        ...
        <div className="mt-4"><Field label="Notes"><textarea value={notes} onChange={e => setNotes(e.target.value)} style={{ ...input, minHeight: 60 }} /></Field></div>
      </div>

      <div className="flex justify-end gap-2">
        <Link href="/invoices" className="px-4 py-2 text-sm" style={{ color: 'var(--text2)' }}>Cancel</Link>
        <button onClick={save} disabled={saving || !clientId || items.every(i => !i.description)} className="px-4 py-2 text-sm rounded-lg" style={{ background: 'var(--purple)', color: '#fff', opacity: saving || !clientId ? 0.5 : 1 }}>
...
const input: React.CSSProperties = { width: '100%', padding: '6px 10px', borderRadius: 6, background: 'var(--bg3)', border: '1px solid var(--border)', color: 'var(--text1)', fontSize: 13 };
