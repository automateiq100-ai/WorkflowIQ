...
export default function DocumentsPage() {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl mb-1" style={{ fontFamily: 'var(--font-dm-serif)', color: 'var(--text1)' }}>Documents</h1>
        <p className="text-sm" style={{ color: 'var(--text2)' }}>Securely store client files in your private vault</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border overflow-hidden" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
          <table className="w-full text-sm">
            <thead><tr style={{ background: 'var(--bg3)' }}><Th>Filename</Th><Th>Client</Th><Th>Category</Th><Th>FY</Th><Th>Size</Th><Th>Uploaded</Th><Th></Th></tr></thead>
            <tbody>
              {filtered.map(d => {
                ...
                return (
                  <tr key={d.id} style={{ borderTop: '1px solid var(--border)' }}>
                    ...
                    <td className="px-4 py-3 text-right">
                      {d.signed_url && <a href={d.signed_url} target="_blank" rel="noopener noreferrer" className="text-xs mr-3" style={{ color: 'var(--purple)' }}>Download</a>}
