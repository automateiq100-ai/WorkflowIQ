...
export default function CalendarPage() {
  ...
  function eventsFor(date: Date) {
    ...
    const stat = events.filter(e => e.date === iso);
    const myTasks = tasks.filter(t => t.due_date === iso);
    return { stat, myTasks };
  }
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm" style={{ color: 'var(--text2)' }}>
            India statutory deadlines + your tasks
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => nav(-1)} className="px-3 py-1.5 rounded text-sm" style={{ background: 'var(--bg3)', color: 'var(--text2)' }}>←</button>
          <div className="text-sm px-3" style={{ color: 'var(--text1)', minWidth: 140, textAlign: 'center' }}>
            {monthName} {year}
          </div>
          <button onClick={() => nav(1)} className="px-3 py-1.5 rounded text-sm" style={{ background: 'var(--bg3)', color: 'var(--text2)' }}>→</button>
        </div>
      ...
      <div className="grid grid-cols-7 gap-1">
        {cells.map((c, i) => {
          ...
          return (
            ...
            >
              <div className="text-xs mb-1" style={{ color: isToday ? 'var(--purple)' : 'var(--text2)', fontWeight: isToday ? 600 : 400 }}>
                {c.date.getDate()}
              </div>
              <div className="space-y-0.5">
                {stat.slice(0, 3).map((e, j) => (
                  <div
                    ...
                    style={{ background: 'var(--bg3)', color: TYPE_COLOR[e.type] }}
                  >
                    {e.title}
                  </div>
                ))}
                {myTasks.slice(0, 2).map(t => {
                  ...
                  return (
                    <div
                      key={t.id}
                      title={`${t.title}${client ? ` · ${client.name}` : ''}`}
                      className="text-[10px] truncate px-1 rounded"
                      style={{ background: 'rgba(76,175,121,0.15)', color: 'var(--green)' }}
                    >
                      ✓ {t.title}
                    </div>
                  );
                })}
                {(stat.length + myTasks.length > 5) && (
                  <div className="text-[10px]" style={{ color: 'var(--text3)' }}>+{stat.length + myTasks.length - 5} more</div>
                )}
              </div>
            </div>
          );
