...
export default function InvoiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function load() {
    const i = await fetch(`/api/practiceiq/invoices/${id}`).then(r => r.json());
    setInv(i.data ?? null);
    if (i.data?.client_id) {
      const c = await fetch(`/api/practiceiq/clients/${i.data.client_id}`).then(r => r.json());
      setClient(c.data ?? null);
    }
    const s = await fetch('/api/practiceiq/settings').then(r => r.json());
  ...
  async function setStatus(status: InvoiceStatus) {
    await fetch(`/api/practiceiq/invoices/${id}`, {
      method: 'PATCH',
  ...
  async function del() {
    ...
    await fetch(`/api/practiceiq/invoices/${id}`, { method: 'DELETE' });
    router.push('/practiceiq/invoices');
  }
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <div className="print:hidden mb-4 flex items-center justify-between">
        <Link href="/practiceiq/invoices" className="text-xs" style={{ color: 'var(--text3)' }}>← All invoices</Link>
        <div className="flex gap-2">
          ...
          <button onClick={printIt} className="px-3 py-1.5 rounded text-xs" style={{ background: 'var(--purple)', color: '#fff' }}>
            Print / PDF
          </button>
          <button onClick={del} className="px-3 py-1.5 rounded text-xs" style={{ background: 'var(--bg3)', color: 'var(--red)' }}>
            Delete
          </button>
        </div>
      </div>
      ...
      <div className="rounded-xl border p-8" style={{ background: 'var(--bg2)', borderColor: 'var(--border)' }}>
        <div className="flex justify-between items-start mb-8">
          <div>
            <div className="text-xl mb-1" style={{ fontFamily: 'var(--font-dm-serif)', color: 'var(--text1)' }}>
              {settings?.firm_name ?? 'Your Firm Name'}
            </div>
        ...
        <div className="ml-auto" style={{ width: 280 }}>
          ...
          <div className="flex justify-between text-base py-2 mt-1" style={{ borderTop: '1px solid var(--border)' }}>
            ...
            <span style={{ color: 'var(--text1)', fontFamily: 'var(--font-dm-serif)' }}>
