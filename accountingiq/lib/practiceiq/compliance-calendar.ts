...
/**
 * Indian statutory deadlines that recur every year.
 * Returns events for a given calendar year (Jan-Dec) — caller filters by visible month range.
 */
export function statutoryEventsForYear(year: number): ComplianceEvent[] {
  const events: ComplianceEvent[] = [];

  // GSTR-1 (11th of every month) — for the previous month's data
  for (let m = 0; m < 12; m++) {
    events.push({
      date: iso(year, m, 11),
      title: 'GSTR-1 due',
      type: 'gst',
      description: 'Outward supplies return for previous month',
    });
  }
  ...
  for (let m = 0; m < 12; m++) {
    events.push({
      ...
      title: 'GSTR-3B due',
      type: 'gst',
      description: 'Summary return + tax payment for previous month',
    });
  }

  // TDS payment (7th of every month except April for March -> 30 Apr)
  for (let m = 0; m < 12; m++) {
    if (m === 3) {
