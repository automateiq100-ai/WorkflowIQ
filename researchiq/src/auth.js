...
async function authenticatedRequest(requestFn) {
    ...
    try {
        ...
        if (status === 401 || status === 403 || status === 409) {
            ...
            // Wait briefly for Centax to clear the old session server-side
            if (status === 409) await new Promise(r => setTimeout(r, 2000));
            try {
                return await requestFn(session);
            } catch (retryErr) {
                const retryStatus = retryErr.response?.status;
                if (retryStatus === 409) {
                    // Second 409: wait longer and try once more
                    console.log('⚠️  Still getting 409, waiting 5s before final retry...');
